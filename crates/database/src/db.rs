...
impl Database {
    ...
    /// Returns `true` if the mailbox exists and has not expired.
    ...
    /// Fetch all emails for a mailbox, newest first.
    ...
    /// number of mailboxes deleted.
    pub async fn delete_expired_mailboxes(&self) -> Result<u64, Error> {
        // Remove emails first to satisfy the FK constraint on older schemas.
        self.client
            .execute(
                "DELETE FROM emails WHERE recipient IN \
                 (SELECT email_address FROM mailboxes WHERE expires_at < NOW())",
                &[],
            )
            .await?;
        ...
        let count = self
            .client
            .execute("DELETE FROM mailboxes WHERE expires_at < NOW()", &[])
            .await?;
